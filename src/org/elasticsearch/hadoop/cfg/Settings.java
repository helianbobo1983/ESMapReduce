 ...
 * Hadoop. Handles internally the fall back to defaults when looking for
...
public abstract class Settings implements InternalConfigurationOptions {
	...
	public String getHost() {
		return StringUtils.hasText(host) ? host : getProperty(ES_HOST,
				ES_HOST_DEFAULT);
	}

	public int getPort() {
		return (port > 0) ? port : Integer.valueOf(getProperty(ES_PORT,
				ES_PORT_DEFAULT));
	}

	public long getHttpTimeout() {
		return TimeValue.parseTimeValue(
				getProperty(ES_HTTP_TIMEOUT, ES_HTTP_TIMEOUT_DEFAULT))
				.getMillis();
	}

	public int getBatchSizeInBytes() {
		return ByteSizeValue.parseBytesSizeValue(
				getProperty(ES_BATCH_SIZE_BYTES, ES_BATCH_SIZE_BYTES_DEFAULT))
				.bytesAsInt();
	}

	public int getBatchSizeInEntries() {
		return Integer.valueOf(getProperty(ES_BATCH_SIZE_ENTRIES,
				ES_BATCH_SIZE_ENTRIES_DEFAULT));
	}

	public boolean getBatchRefreshAfterWrite() {
		return Booleans.parseBoolean(getProperty(ES_BATCH_WRITE_REFRESH,
				ES_BATCH_WRITE_REFRESH_DEFAULT));
	}

	public long getScrollKeepAlive() {
		return TimeValue.parseTimeValue(
				getProperty(ES_SCROLL_KEEPALIVE, ES_SCROLL_KEEPALIVE_DEFAULT))
				.getMillis();
	}

	public long getScrollSize() {
		return Long
				.valueOf(getProperty(ES_SCROLL_SIZE, ES_SCROLL_SIZE_DEFAULT));
	}
	...
	public boolean getIndexAutoCreate() {
		return Booleans.parseBoolean(getProperty(ES_INDEX_AUTO_CREATE,
				ES_INDEX_AUTO_CREATE_DEFAULT));
	}

	public boolean getIndexReadMissingAsEmpty() {
		return Booleans.parseBoolean(getProperty(
				ES_INDEX_READ_MISSING_AS_EMPTY,
				ES_INDEX_READ_MISSING_AS_EMPTY_DEFAULT));
	}

	public String getOperation() {
		return getProperty(ES_WRITE_OPERATION, ES_WRITE_OPERATION_DEFAULT)
				.toLowerCase();
	}
	...
	public boolean getUpsertDoc() {
		return Booleans.parseBoolean(getProperty(ES_UPSERT_DOC,
				ES_UPSERT_DOC_DEFAULT));
	}

	public String getTargetUri() {
		String address = getProperty(INTERNAL_ES_TARGET_URI);
		return (StringUtils.hasText(address) ? address : new StringBuilder(
				"http://").append(getHost()).append(":").append(getPort())
				.append("/").toString());
	}
	...
	public void save() {
		...
		Assert.hasText(resource, String.format(
				"No resource (index/query/location) ['%s'] specified",
				ES_RESOURCE));
